...
const createReviewSchema = z.object({
    ...
    review_datetime: z.string().transform((val) => (val ? new Date(val) : null)).nullable().optional(),
    reply_content: z.string().nullable().optional(),
    replied_at: z.string().transform((val) => (val ? new Date(val) : null)).nullable().optional(),
    app_version: z.string().max(100).nullable().optional(),
    timestamp_unix: z.number().transform((val) => (val ? BigInt(val) : null)).nullable().optional(),
